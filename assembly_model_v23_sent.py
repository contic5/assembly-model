...
def determinebestpart(currentpart,currentarm,selectedparts,parts):
    ...
    for j in range(len(parts)):
        ...
        if(partscore>bestpartscore):
            ...
            bestpart=j
    '''
    if(bestpart!=-1 and parts[bestpart].weight>=veryencumbered and currentarm.name=='Human' and 'NOTCSK' in priorityfileusing and autowaitheavy):
        bestpart=-1
    '''
    
    #print('Best part is',bestpart,'with bestpartscore',bestpartscore)
    if(bestpart!=-1):
...
def main():
    ...
    if(usingfiles):
        ...
        robotmin=[-2*change,0*change,change,0*change,0*change]
        robotmax=[2*change,5*change,3*change,2*change,0*change]
